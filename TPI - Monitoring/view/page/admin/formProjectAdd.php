...
<div class="flex bg-gray-100 items-center justify-center mb-32">
    <div class="grid bg-white rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2 mt-20 mb-32">
        <form class='mt-5 mb-5' method='post' action='index.php?controller=admin&action=add'>
            ...
            <div class="grid grid-cols-1 mt-5 mx-7">
                ...
                <input class="py-2 px-3 rounded-lg border-2 border-teal-300 mt-1 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent" id="proName" name="proName" type="text" placeholder="ICT101 - Web" />
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                <div class="grid grid-cols-1">
                    ...
                    <input class="py-2 px-3 rounded-lg border-2 border-teal-300 mt-1 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent" id="proStart" name="proStart" type="text" placeholder="1970-01-01" />
                </div>
                <div class="grid grid-cols-1">
                    <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">Date de fin | Format : yyyy-mm-dd</label>
                    <input class="py-2 px-3 rounded-lg border-2 border-teal-300 mt-1 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent" id="proEnd" name="proEnd" type="text" placeholder="2038-01-19" />
                </div>
            </div>
            <div class="grid grid-cols-1 mt-5 mx-7">
                <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">Attribuer le projet à une personne</label>
                <input class="py-2 px-3 rounded-lg border-2 border-teal-300 mt-1 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent" id="useEmail" name="useEmail" type="text" placeholder="xxx@eduvaud.ch" />
            </div>
            <div class="grid grid-cols-1 mt-5 mx-7">
                ...
                <select id="0clagro" name="0clagro" class="py-2 px-3 rounded-lg border-2 border-teal-300 mt-1 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent">
                    <?php
                        foreach ($classes as $class){
                            echo '<option value=\''.$class['idClass'].'cla\'>'.$class['claName'].'</option>';
                        }
                        foreach ($groups as $group){
                            echo '<option value=\''.$group['idGroup'].'gro\'>'.$group['groName'].'</option>';
                        }
                    ?>
                </select>
